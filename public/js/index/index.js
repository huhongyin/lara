...
function bigImg(url)
{
    ...
    var modalWidth = bodyWidth * 0.9;
    var modalHeight = bodyHeight * 0.9;
    layer.open({
  	type: 1,
  	title: false,
  	closeBtn: 0,
  	area: [modalWidth + 'px', modalHeight + 'px'],
  //	skin: 'layui-layer-nobg', //没有背景色  
	shadeClose: true,
        content: '<div style="width:100%;height:100%;text-align:center;"><img style="" src="'+url+'"></div>'
    });
}
			...
			for(var i in photoes){
				photoesStr += '<div onclick="bigImg(\''+photoes[i]['realPath']+'\')" class="col-md-3 col-sm-4 col-xs-6 ui-sortable-handle" id="'+photoes[i]['id']+'_div">' +
									'<div class="album-image">'+
										...
										'</a>'+
										'<a href="#" class="name">'+
											'<em>'+date('Y-m-d', photoes[i]['created_at'])+'</em>'+
										'</a>'+
